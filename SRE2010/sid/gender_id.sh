...
if [ $stage -le -1 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/gselect.JOB.log \
    ...
    fgmm-gselect --gselect=ark,ns,cs:- --n=$num_gselect2 $ubmdir/final.ubm \
...
if [ $stage -le 0 ]; then
  $cmd JOB=1:$nj $dir/log/get_male_logprob.JOB.log \
    fgmm-global-get-frame-likes --average=true \
     "--gselect=ark,ns,cs:gunzip -c $dir/gselect.JOB.gz|" $male_ubmdir/final.ubm \
      "$feats" ark,t:$dir/male_logprob.JOB || exit 1;
fi
if [ $stage -le 1 ]; then
  $cmd JOB=1:$nj $dir/log/get_female_logprob.JOB.log \
    fgmm-global-get-frame-likes --average=true \
     "--gselect=ark,ns,cs:gunzip -c $dir/gselect.JOB.gz|" $female_ubmdir/final.ubm \
