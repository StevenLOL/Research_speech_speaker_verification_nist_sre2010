...
nj=6   # this is the number of separate queue jobs we run, but each one 
...
## Set up features.
feats="ark,ns,cs:add-deltas scp:$sdata/JOB/feats.scp ark:- | apply-cmvn-sliding --norm-vars=false --center=true --cmn-window=300 ark:- ark:- | select-voiced-frames ark:- scp,ns,cs:$sdata/JOB/vad.scp ark:- |"
...
if [ $stage -le -1 ]; then
  ...
  $cmd JOB=1:$nj_full $dir/log/gselect.JOB.log \
    ...
    fgmm-global-gselect-to-post --min-post=$min_post $dir/final.ubm "$feats" \
       ark,ns,cs:- "ark:|gzip -c >$dir/post.JOB.gz" || exit 1;
else
...
while [ $x -lt $num_iters ]; do
  if [ $stage -le $x ]; then
    ...
    for j in $(seq $nj_full); do
      Args[$j]=`echo "ivector-extractor-acc-stats --num-threads=$num_threads --num-samples-for-weights=$num_samples_for_weights $dir/$x.ie '$feats' 'ark,ns,cs:gunzip -c $dir/post.JOB.gz|' -|" | sed s/JOB/$j/g`
    done
